...
}elseif(isset($_SESSION['info_ins'])){
    echo '<div class="alert alert-success" role="alert">
    <span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"></span>
    <span class="sr-only">Registered:</span>
    '.$_SESSION["info_ins"].'
    </div>';
    unset($_SESSION['info_ins']);
}

if(!isset($_SESSION['site'])) {
    echo '<div class="row"><div class="col-md-offset-3 col-md-6" id="form_ins">';
    echo '<form method="post" action="../Scripts/verificationinscription.php">';
    echo '<label>Identifiant : <input type="text" name="id"></label>';
    echo '<label>Mot de Passe : <input type="password" name="mdp"></label>';
    echo '<label>Vérification Mot de Passe : <input type="password" name="vmdp"></label><br><br>';
    echo '<button class="btn btn-success" type="submit">Valider <span class="glyphicon glyphicon-ok"></span></button>';
    echo '</div></div>';
}else{;
